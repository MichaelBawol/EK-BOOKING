// api/bookings.js
import { preflight, allow } from './cors';
...
async function sendEmails(booking) {
  ...
  if (!host || !user || !pass || !from || !admin) return { sent: false, reason: 'smtp_not_configured' };
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Optional: store in Vercel KV if configured
      let stored = false;
      try {
      ...
      try { email = await sendEmails(booking); } catch (e) { console.warn('Email send failed', e); email = { sent: false, reason: 'send_failed' }; }
